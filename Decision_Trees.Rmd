...
data_frame <- read.csv("fraud_data.csv")
...
```{r}
tree_grid <- tibble(cost_complexity = 10^seq(from = -6, to = -1, length.out = 20))
tree_grid
```
...
tree_tune_class <- tree_wf %>% 
  tune_grid(resamples = cv_folds,
            grid = tree_grid,
            metrics = class_metrics)

tree_bf_class <- tree_tune_res
tree_bf_class_metrics <- tree_bf_class %>% collect_metrics()

save(tree_bf_class, file="tree_bf_result_train.RData")
load("tree_bf_result_train.RData")
save(tree_bf_class_metrics, file="tree_bf_metrics_train.RData")
...
```{r}
tree_class_metrics <- tree_tune_class %>% collect_metrics()
tree_class_metrics
```
...
```{r}
tree_class_metrics %>% filter(.metric %in% c("sens", "spec")) %>% 
...
```{r}
tree_class_metrics %>% filter(.metric == "accuracy") %>% 
  ggplot(aes(x = cost_complexity, y = mean, 
             ymin = mean - std_err, ymax = mean + std_err)) +
  geom_errorbar(width = 0.1) + 
  geom_point() + scale_x_log10()
...
```{r}
tree_metrics_sub <- tree_class_metrics %>% 
  pivot_wider(names_from = ".metric", 
              values_from = c("mean", "std_err")) %>% 
  filter(mean_sens > 0.64, mean_sens < 0.76) %>% 
...
```{r}
tree_selected <- tree_metrics_sub %>% filter(.config == "Preprocessor1_Model16")
tree_selected
```
...
```{r}
rpart_pruned <- rpart::prune(rpart_fit, cp = 0.004)
...
```{r}
rpart_test_bf_pred <- predict(rpart_pruned, newdata = fr_test, type = "class")

rpart_test_bf_metrics <- 
  fr_test %>% mutate(rpart_pred = rpart_test_bf_pred) %>% 
