...
```{r}
tree_metrics_sub <- tree_nonbenf_tune_metrics %>% 
...
tree_bf_selected <- tree_metrics_sub %>% filter(.config == "Preprocessor1_Model16")
tree_bf_selected
```
...
```{r}
rpart_pruned <- rpart::prune(rpart_fit, cp = 0.0040818)
rpart.plot(rpart_pruned)
plot(as.party(rpart_pruned))


rpart_bf_pruned <- rpart::prune(rpart_bf_fit, cp = 0.0041511)
...
```{r}
mcc_nb_metric <- nonbenf_test %>% mutate(rpart_pred = rpart_test_pred) %>%  yardstick::mcc(nonbenf_test, truth = misstate, estimate = rpart_pred, na_rm = TRUE, event_level = "second")


mcc_b_metric  <- benf_test %>% mutate(rpart_bf_pred = rpart_bf_test_pred) %>%
  yardstick::mcc(truth = misstate, estimate = rpart_bf_pred, event_level = "second")
