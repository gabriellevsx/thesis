...
```{r}
tree_recipe <- recipe(misstate ~ ., data = nonbenf_train) %>% 
  step_rm(period) %>%
  step_dummy(fyear) %>%
...
tree_benf_recipe <- recipe(misstate ~ ., data = benf_train) %>% 
  step_rm(period) %>%
  step_dummy(fyear, bf_1, bf_2, bf_3, bf_agr) %>%
  step_downsample(misstate)
...
```{r}
rpart_pruned <- rpart::prune(rpart_fit, cp = 0.005)
rpart.plot(rpart_pruned)
plot(as.party(rpart_pruned))


rpart_bf_pruned <- rpart::prune(rpart_bf_fit, cp = 0.005)
