...
data_sample <- data_frame[sample(nrow(data_frame), 50000), ]
```
...
```{r}
set.seed(466581)
fr_split <- initial_split(data = data_sample, prop = 0.7, 
                          strata = "misstate")
...
```{r}
set.seed(91231)
fr_valid <- validation_split(fr_train, prop = 7/10, strata = misstate)
```
...
```{r}
knn_regr_tune_grid <- tibble(neighbors = 1:6*2 - 1)
knn_regr_tune_grid
```
...
```{r}
fr_train_baked <- knn_regr_recipe %>% prep(fr_train) %>% bake(fr_train)
fr_train_baked %>% head()
