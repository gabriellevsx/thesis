...
knn_nonbenf_tune_res <- knn_nobenf_wf %>% 
  tune_grid(resamples = nonbegnf_cv, 
...
We specified metrics that are specific to the evaluation of (binary) classification models. 
...
These metrics can also be plotted. 

```{r}
knn_nonbenf_tune_metrics %>% 
...
```{r}
knn_nonbenf_tune_res %>% 
  show_best("roc_auc", n = 3) %>% 
  arrange(neighbors)
```
From the graphs above, we must choose the optimal number of neighbors for our model. 
...
```{r}
knn_nonbenf_best_model <- select_best(knn_nonbenf_tune_res, metric = "roc_auc")
...
Now that our model has been tuned, we can test its performance on the test set. 
...
write.csv(knn_nonbenf_metrics, "knn_nonbenf_res.csv")
```
