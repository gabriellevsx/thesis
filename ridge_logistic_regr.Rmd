...
ridge_1se_b_model <- select_by_one_std_err(ridge_b_tune, metric = "roc_auc", desc(penalty))
ridge_1se_b_model
...
ridge_wf_b_tuned <- 
  logreg_b_wf %>% 
  finalize_workflow(ridge_1se_b_model)
ridge_wf_b_tuned
```
...
ridge_last_fit <- ridge_wf_tuned %>% 
  last_fit(nonbenf_split, metrics = mcc)
...
nonbenf_pred <- nonbenf_test %>% mutate(ridge_pred = ridge_test_pred$.pred_class)
benf_pred <- benf_test %>% mutate(ridge_pred = ridge_test_b_pred$.pred_class)

  
mcc_b_metric <- yardstick::mcc(benf_pred, truth = misstate, estimate = ridge_pred, event_level = "second")
mcc_nb_metric <- yardstick::mcc(nonbenf_pred, truth = misstate, estimate = ridge_pred, event_level = "second")
